...
interface GetClinicAvailabilityParams {
  clinicId: string;
...
export async function getClinicAvailability({
  ...
  const bookedIntervals = appointments
    .filter((appointment) => {
      ...
      return dayjs(appointment.date).tz(clinic.timezone).isSame(date, "day");
    })
    .map((appointment) => {
  ...
  if (hasConfiguredHours && dayOperatingHours) {
    ...
    const clinicStartMinute = Number(dayOperatingHours.startTime.split(":")[1]);
  ...
  if (clinic.hasLunchBreak && clinic.lunchBreakStart && clinic.lunchBreakEnd) {
    lunchStart = dayjs()
      .tz(clinic.timezone)
